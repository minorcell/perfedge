...
在这个例子中,当点击"更新无关状态"按钮时,父组件重新渲染,但由于传给`ChildWithMemo`的props没有变化,它不会重新渲染。而`ChildWithoutMemo`每次都会重新渲染。
...
在这个例子中：
- 如果不使用`useCallback`，每次`TodoApp`渲染时都会创建新的`handleClick`函数
- 即使使用了`memo`，由于函数引用变化，`ExpensiveList`仍会重新渲染
- 使用`useCallback`后，函数引用保持不变，`memo`才能真正发挥作用
